...
static float createVelocity(const float& xa, const float& ya, const float& xb, const float& yb) {
	if (ya == yb) {
		if (xa > xb) return pow(pow(xa - xb, 2) + pow(ya - yb, 2), 0.5f) / 2;
		if (xa < xb) return -pow(pow(xa - xb, 2) + pow(ya - yb, 2), 0.5f) / 2;
		if (xa == xb) return 0;
	}
	if (ya > yb) return -pow(pow(xa - xb, 2) + pow(ya - yb, 2), 0.5f) / 2;
	return pow(pow(xa - xb, 2) + pow(ya - yb, 2), 0.5f) / 2;
}
